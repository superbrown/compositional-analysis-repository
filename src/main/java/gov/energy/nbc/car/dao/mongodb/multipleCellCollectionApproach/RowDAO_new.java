...
public class RowDAO_new extends DAO implements IRowDAO {

    public static final String COLLECTION_NAME = "row";

    protected Logger log = Logger.getLogger(this.getClass());
    private Map<String, CellDAO_new> cellDAOs;

    public RowDAO_new(ISettings settings) {
    ...
    public List<ObjectId> add(ObjectId datasetId, DatasetDocument datasetDocument, RowCollection data) {
        ...
        for (Row row : data) {
            ...
            for (String columnName : row.getColumnNames()) {
                CellDAO_new cellDAO = getCellDAO(columnName);
                cellDAO.add(rowId, row.get(columnName));
            }
        }
...
//                append(CellDocument.ATTR_KEY__COLUMN_NAME, searchCriterion.getName()).
    ...
    @Override
    public CellDAO_new getCellDAO(String columnName) {

        CellDAO_new cellDAO = cellDAOs.get(columnName);

        if (cellDAO == null) {
            cellDAO = new CellDAO_new(columnName, settings);
